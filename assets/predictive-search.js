...
document.addEventListener('click', function (event) {
  ...
  const keyword = chip.dataset.search || chip.textContent.trim();

  // 検索インプットを取得（テーマによってセレクタは調整してください）
  const input =
    document.querySelector('input[name="q"][data-predictive-search-input]') ||
  ...
  // predictive search を発火させる（入力イベント）
